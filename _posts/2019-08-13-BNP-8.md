...
$N_{(j),n}$은 partition $\mathcal P_n$의 $j$번째로 큰 set의 크기이다. 따라서, 가능한 수많은(엄밀하게는 uncountably many) $x$들에 대해, $N_{(j),n}$은 $ \left\{ i \in \mathbb N_n : X_i = x \right\} $의 cardinality, $N_n(x)$를 크기 내림차순으로 정렬했을 때 그 중 $n$번째 수라는 것을 알 수 있다. 따라서 ergodic law of large numbers에 의해,
