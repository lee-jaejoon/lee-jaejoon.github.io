---
layout: post
title: "Chapter 5. Discrete Random Structures"
...
$N_{(j),n}$은 partition $\mathcal P_n$의 $j$번째로 큰 set의 크기이다. 따라서, 가능한 수많은(엄밀하게는 uncountably many) $x$들에 대해, $N_{(j),n}$은 $N_n(x) = \#\{ i \in \mathbb N_n : X_i = x \}$를 크기 내림차순으로 정렬했을 때 그 중 $n$번째 수라는 것을 알 수 있다. 따라서 ergodic law of large numbers에 의해,

$$
n^{-1} N_n(x) = n^{-1} \#\{ i \in \mathbb N_n : X_i = x \} \longrightarrow \mathbb P (X_1 = x \vert P), \text{ a.s. }
$$
...
$$
p^{\tilde \Pi}(\mathbf n) = \prod_{i=2}^{n} p_{{\tilde \Pi}(i)}\Big(\mathbf n({\tilde \Pi}_{i-1}) \Big) = p_{{\tilde \Pi}(2)}\Big(\mathbf n({\tilde \Pi}_1) \Big) \cdot p_{{\tilde \Pi}(3)}\Big(\mathbf n({\tilde \Pi}_2) \Big) \cdot \text{ } \cdots \text{ }\cdot p_{{\tilde \Pi}(n)}\Big(\mathbf n({\tilde \Pi}_{n-1}) \Big)
$$
