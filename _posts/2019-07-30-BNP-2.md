 ...
 * $\alpha \neq \alpha ' \text{ & } G_0 \neq G_0 '$인 경우, $\text{DP}(\alpha)$와 $\text{DP}(\alpha ')$는 서로 다른 expectation을 갖는다.
 * $\alpha \neq \alpha ' \text{ & } G_0 = G_0 '$인 경우, $\alpha_0 \neq \alpha_0 '$이다. 그러면 임의의 $A$에 대하여 $ 0 < G_0(A) = G_0 '(A) < 1$를 가정하면 $\text{DP}(\alpha)$와 $\text{DP}(\alpha ')$는 서로 다른 variance를 갖는다.
...
또한, 2번 성질에서처럼 $i=1,\cdots,k$에 대해 $\frac{Z_i}{\sum^{k}_{j=1} Z_j}  \perp\!\!\!\perp \sum^{k}_{j=1} Z_j$인 것은 도출 과정 중간에 얻은 density 식이 factorize되는 것에서 알 수 있다.
...
$$
\begin{align*}
\Big( \frac{P(A_{00})}{P(A_0)} ,\frac{P(A_{01})}{P(A_0)}\Big) 
...
$$
\begin{align*}
\Big( \frac{P(A_{10})}{P(A_1)}, \frac{P(A_{11})}{P(A_1)}\Big) 
&\stackrel{d}{=} \Big( \frac{Z_3}{Z_3+Z_4} , \frac{Z_4}{Z_3+Z_4} \Big)\\
...
$j \leq 1$에 대해, $W_j ' = V_{j+1} \prod_{l=2}^{j}(1-V_l)$와 $\theta_j ' = \theta_{j+1}$을 정의하자. $j \leq 1$에 대해, 다음이 만족한다.
