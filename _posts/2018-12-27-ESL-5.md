...
어떤 부분집합 $B= \\{ b_1, b_2, ...,b_n \\} \subset V$의 원소들의 선형결합으로 $B$가 속한 벡터공간 $V$의 모든 원소들을 나타낼 수 있다면, 우리는 $B$가 $V$를 span한다고 나타내거나, 혹은 $B$를 $V$의 spanning set이라고 부른다. 한 벡터공간을 span하는 집합은 무수히 많을 수 있다. **Basis**는 t선형독립이면서 어떤 벡터공간을 span하는 set, 즉 어떤 벡터 공간의 **minimal spanning set**을 말한다.

예를 들면, $a\cdot1+b\cdot x+c\cdot x^2+d\cdot x^3$의 꼴로 3차 이하의 다항식을 모두 나타낼 수 있고,  $\\{ 1,x,x^2,x^3 \\}$는 서로 선형독립이기 때문에, $ \\{ 1,x,x^2,x^3 \\} $는 3차 이하의 모든 다항식들의 집합 $P_3$의 **basis**이다. (물론 이것이 유일한 basis는 아니다.)

위의 linear model의 예의 경우 true function $f(X)$를 나타내는 **basis**는 각 input feature $ \\{ 1,X_1,X_2,...,X_p \\} $가 된다.  
...
데이터를 polynomial로 fit하는 경우 정의역의 양 끝 값 주변에서 오차가 커질 가능성이 있고, 이는 아래 그림과 같이 polynomial의 차수가 올라갈 수록 그 정도가 더 심해진다. 각 구간마다 polynomial이 되는 piecewise polynomial 혹은 spline 역시 같은 문제를 안고 있다. **Natural cubic spline**은 이와 같은 문제를 해결하고자 한 예이다.
...
## R Example: Spline & Natural spline 

만약 우리가 가진 데이터를 basis expansion을 이용한 모형으로, 그 중에서도 (natural) spline을 이용하여 나타내고 싶다면, 우리는 spline의 order(차수), 그리고 knot의 수와 위치를 결정하여야 한다. 이 예제에서는 cubic spline(order-$4$ spline)을 R을 이용하여 나타내보고자 한다.  
...
```
<p align="center">
<img src="https://user-images.githubusercontent.com/45325895/50607547-ee16b300-0f0c-11e9-97f3-fc18068c59a0.png">
</p>

```r
df <- 4  					#degrees of freedom = 4
BS <- bs(x, degree=3, df=df)
NS <- ns(x, df=df)				#basis matrix 생성
BS.fit = lm( y ~ BS )
NS.fit = lm( y ~ NS )				#새로운 basis로 fitting
plot(x, y)
lines( x, fitted(BS.fit), col='red')
lines( x, fitted(NS.fit), col='blue')
```

<p align="center">
<img src="https://user-images.githubusercontent.com/45325895/50608592-6e8ae300-0f10-11e9-8559-92280c5fe9f5.png">
</p>
