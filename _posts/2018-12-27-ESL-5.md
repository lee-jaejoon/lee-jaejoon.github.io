...
1. $H$와 $S_\lambda$는 대칭행렬, positive semidefinite행렬이라는 특징이 있다.
2. $H$는 $H^2=HH=H$, 즉 idempotent한 특징이 있다. $S_\lambda$는 $S_\lambda^2=S_\lambda S_\lambda \preceq S_\lambda$인 성질이 있다. 이는 $S_\lambda $가 $S_\lambda S_\lambda$보다 어떤 positive semidefinite행렬만큼 더 크다는 것을 의미한다. 이는 smoother matrix가 shrinking(줄어드는, 쪼그라드는)한 특징을 가지고 있기 때문인데, 이에 대해서는 뒤에 그 의미와 이유를 더 설명하겠다.
3. rank란 어떤 linear operator의 column space의 차원, 다시 말해서 linear operator의 결과로 이루어진 공간(치역:range)의 차원을 의미한다. 회귀모형이 $p$개의 항을 가질 때 $X$는 $p$개의 열로 이루어진 행렬이 되며, $H$는 $p$의 rank를 갖는다. $S_\lambda$는 $N$의 rank를 갖는다.
