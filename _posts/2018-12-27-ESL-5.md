...
위 식을 최소화하는 $\theta$를 찾기 위해 $\theta$에 대한 일계조건을 풀면 다음과 같다.
...
따라서 smoothing spline의 fitted value, $\hat{ y}$, 는 아래와 같이 $y$에 $S_\lambda=N \Big(  N^T N+ \lambda \Omega_N \Big) ^{-1}  N^T$를 곱한형태이다. 이 때, 행렬 $S_\lambda$를 **Smoother matrix**라고 부른다.
...
위에서와 같이 $\lambda$가 사전에 결정되어 있다면, $y$에 $S_\lambda$라는 행렬을 곱함으로써 얻은 Smoothing spline은 $y$에 대해 linear smoother이다.(Matrix multiplication은 linear transformation) 그리고 이 Smoother matrix는 $x_i$와 $\lambda$에 의해 결정된다. 
