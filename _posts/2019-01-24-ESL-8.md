...
그와 반대로, Frequentist의 통계적 추론 방법인 maximum likelihood는 관측된 $\mathbf{Z}$로 보아 가장 Likely한 parameter 값, $\hat \theta_{MLE}$를 구하고, $Pr(z^{new} \mid \theta =\hat \theta_{MLE})$를 이용해 예측을 수행한다. 이는 위에서 언급한 Bayesian inference와는 다르게, $\theta$가 uncertain하다는 것을 반영하지 않은 접근인 것이다.  
