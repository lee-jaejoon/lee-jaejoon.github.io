...
Neural network를 **훈련(train)**시키는 것은 다른 통계모형이나 머신러닝 기법들과 크게 다르지 않다. 위 절에서 언급한 것과 같이, 모형에 맞는 **cost function을 특정하고**, 그 **cost function을 최소화하는 parameter의 값을 구하는 것**이다. 다만 다른 점은, neural network의 nonlinearity 때문에 통계학에서처럼 convex optimization이나 선형방정식의 해 도출을 통해 cost function을 최소화할 수 없다는 것이다. 그래서 우리는 **gradient descent** 혹은 그를 발전시킨 **iterative, gradient-based 최적화** 방법을 통해 neural network를 훈련시킨다. 이러한 gradient-based 최적화에 대한 내용은 Chapter 8에서 자세하게 다룰 예정이다. 지금은 우리의 training 알고리즘이 **cost function의 gradient를 사용하여 이루어진다**는 점만 인지하고 넘어가도 충분하다. 이 장에서는 cost function이 무엇인지, 또 우리가 다루는 output의 분포에 따라 cost function이 어떻게 달라지는지에 대해 알아볼 것이다.  

그에 앞서 몇 가지 개념을 확실히 짚고 넘어갈 필요가 있다. 통계학을 전공한 사람이라면 loss function에 대해서 들어보았을텐데, 이는 cost function과 어떤 관계가 있을까? 엄밀하게 정의된 개념들은 아니지만 주로 다음과 같은 의미로 구별되어 쓰인다.  

**Loss function:**  
  
 * 한 data point 위에서 정의되는 함수로, fit이나 prediction이 어긋난 정도를 측정하는 함수이다.
 * 예를 들면 선형회귀분석에서는 주로 square loss, $l(y_i, f(x_i ; \theta))=(y_i-f(x_i ; \theta))^2$를 loss function으로 사용한다.
  
**Cost function:**  
  
 * loss function이 한 data point에 대해 정의된다면, 보통 cost function은 좀 더 general한 의미로 쓰인다. 
 * 모형의 최적 parameter 값 결정을 위해 최소화하고자 하는 target 함수를 의미한다.
 * 위의 square loss를 이용한 선형회귀분석을 예로 들면, training set의 loss function을 모두 더한 Residual Sum of Square, $RSS(\theta)=\sum_{i=1}^{N} (y_i-f(x_i ; \theta))^2$가 cost function이 된다.
 * ridge regression, lasso, smoothing spline 등, 모형에 따라 penalty항이 추가된 형태의 cost function을 갖는 경우도 있다.  
# 6.3 Hidden Units
