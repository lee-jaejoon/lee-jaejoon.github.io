...
로그함수는 concave 함수이므로 Jensen 부등식에 의해, $\log \mathbb{E}_\tilde P \Big[ \frac{P(\mathbf{Z},\mathbf{Z}^m;\theta')}{\tilde P(\mathbf{Z}^m)}\Big] \ge \mathbb{E}_\tilde P \Big[ \log \Big[  \frac{P(\mathbf{Z},\mathbf{Z}^m;\theta')}{\tilde P(\mathbf{Z}^m)} \Big] \Big]$가 만족한다. 즉, $F(\theta',\tilde P)$는 최대화하고자 하는 $\log P(\mathbf{Z};\theta')$의 lower bound라는 것을 알 수 있다.
