 ...
 * AdaBoost.M1을 수행하는 것은 forward stagewise additive modeling 하에서 exponential loss criterion을 최소화한 것과 다.
...
그렇다면 additive modeling에서 exponential loss를 최소화하는 것은 어떤 의미가 있을까? Exponential loss를 최소화하는 것은 무엇을 추정하는 문제일까? 이는 exponential loss의 population minimizer를 고려해보면 쉽게 보일 수 있다 (Friedman et al., 2000).  
