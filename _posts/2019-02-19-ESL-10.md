...
여기서 $\mathbf{f}_{0}=\mathbf{h}_{0}$은 initial guess를 의미하고, $\mathbf{f}_{m}$은 이전 iteration까지의 guess, $\mathbf{f}_{m-1}$를 바탕으로 새 guess $\mathbf{h}_{m}$을 결정하는 방식으로 update된다. 이 guess를 어떻게 결정하는지에 따라 다양한 numerical optimization 방법이 있다. Steepest descent 방법은 아래와 같은 방법으로 새 guess $\mathbf{h}_{m}$를 결정한다.
