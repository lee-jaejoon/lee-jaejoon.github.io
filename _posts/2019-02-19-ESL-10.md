...
여기서 $\mathbf{f}_0=\mathbf{h}_0$은 initial guess를 의미하고, $\mathbf{f}_m$은 이전 iteration까지의 guess, $\mathbf{f}_{m-1}$를 바탕으로 새 guess $\mathbf{h}_m$을 결정하는 방식으로 update된다. 이 guess를 어떻게 결정하는지에 따라 다양한 numerical optimization 방법이 있다. Steepest descent 방법은 아래와 같은 방법으로 새 guess $\mathbf{h}_m$를 결정한다.
...
어떤 점에서의 Gradient는 그 점에서 함수가 가장 가파르게 증가하는 방향을 가리키는 벡터이므로, negative gradient인 $-\mathbf{g}_m$은 이전 step까지의 예측 결과인 $\mathbf{f}=\mathbf{f}_{m-1}$에서 $L(\mathbf{f})$가 가장 가파르게 감소하는 방향을 나타낸다. 또한, $\rho_m$은 $-\mathbf{g}_m$ 방향으로 움직였을 때 가장 $L(\mathbf{f})$가 크게 감소하는 step length를 의미한다.
